  ...
  return (
    <>
      ...
      <div className="fixed top-0 right-0 w-[380px] h-full bg-white dark:bg-neutral-900 shadow-2xl z-50 animate-slide-in-right overflow-hidden flex flex-col">
        {/* Clean Header */}
        <div className="relative bg-primary-600 p-4 text-white">
          <div className="relative flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div>
                <h2 className="text-base font-[400]">Theme Studio</h2>
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="w-6 h-6 rounded-lg hover:text-white/80 backdrop-blur-sm flex items-center justify-center transition-colors"
              >
                <RotateCcw className="w-4.5 h-4.5 hover:-rotate-180 transition-transform duration-500" />
              </button>
              <button
                onClick={toggleCustomizer}
                className="w-6 h-6 group rounded-lg hover:text-white/80 backdrop-blur-sm flex items-center justify-center transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <section className="space-y-3">
            <div className="flex items-center gap-1.5 text-sm font-medium text-neutral-700 dark:text-neutral-300 tracking-wide">
              <Palette className="w-4 h-4 text-white opacity-50" />
              <span>Primary Color</span>
            </div>

            {/* Large Color Preview */}
            <div className="p-6 rounded-xl border-2 border-neutral-200 dark:border-neutral-700 bg-gradient-to-br from-neutral-50 to-white dark:from-neutral-800 dark:to-neutral-900 space-y-3 hidden">
              <div className="text-center">
                <div
                  className="inline-flex items-center justify-center w-16 h-16 rounded-2xl shadow-xl transition-all duration-300"
                  style={{ backgroundColor: config.theming.primaryColor }}
                >
                  <div className="text-white font-bold text-xs">LIVE</div>
                </div>
            ...
            <div className="grid grid-cols-8 gap-2">
              {colorPresets.map((preset) => (
                <button
                  ...
                  className="p-1.5 group relative border rounded-lg"
                  style={{ borderColor: preset.value }}
                >
                  <div
                    className={cn(
                      "h-full rounded-lg transition-all relative",
                      config.theming.primaryColor === preset.value ? "scale-105" : "hover:scale-105"
                    )}
                  >
                    <div
                  ...
                  <div className="hidden absolute -bottom-0 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <span className="text-[10px] text-white whitespace-nowrap">{preset.name}</span>
                  </div>
                </button>
              ))}
              {/* Custom Color Picker */}
              <button
                ...
                className="w-full p-2 rounded-lg border-2 border-dashed border-neutral-300 dark:border-neutral-700 hover:border-primary-500 dark:hover:border-primary-500 transition-colors flex items-center justify-center gap-2 text-sm font-medium text-neutral-600 dark:text-neutral-400"
              >
            ...
            {showCustomColor && (
              <div className="p-4 bg-neutral-50 dark:bg-neutral-800 rounded-lg space-y-3">
                ...
                <input
                  ...
                  onChange={(e) => updateTheming({ primaryColor: e.target.value })}
                  placeholder="#6366f1"
                  className="w-full px-3 py-2 rounded-lg border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 dark:text-white/75 text-sm font-mono"
                />
              </div>
            )}
          </section>
          ...
          <section className="space-y-3">
            <div className="flex items-center gap-1.5 text-sm font-medium text-neutral-700 dark:text-neutral-300 tracking-wide">
              <Sun className="w-4 h-4 text-white opacity-50" />
          ...
          <section className="space-y-3">
            <div className="flex items-center gap-1.5 text-sm font-medium text-neutral-700 dark:text-neutral-300 tracking-wide">
              <Layout className="w-4 h-4 text-white opacity-50" />
              <span>Layout</span>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="p-4 bg-neutral-50 dark:bg-neutral-800 rounded-xl space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={() =>
                      updateLayout({
                        navbarType: config.layout.navbarType === "sticky" ? "static" : "sticky",
                      })
                    }
                    className={cn(
                      "relative inline-flex h-6 w-11 items-center rounded-full transition-colors",
                      config.layout.navbarType === "sticky"
                        ? "bg-primary-600"
                        : "bg-neutral-300 dark:bg-neutral-600"
                    )}
                  >
                    <span
                      className={cn(
                        "inline-block h-4 w-4 transform rounded-full bg-white transition-transform shadow-sm",
                        config.layout.navbarType === "sticky" ? "translate-x-6" : "translate-x-1"
                      )}
                    />
                  </button>
                </div>
              ...
              <div className="p-4 bg-neutral-50 dark:bg-neutral-800 rounded-xl space-y-2">
                ...
                <p className="text-xs text-neutral-500 pl-6">Dark sidebar with light content</p>
              </div>
